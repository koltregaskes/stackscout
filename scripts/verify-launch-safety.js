...
function extractIssueDate() {
  ...
  const issueMatch = indexHtml.match(/Issue (\d{4}-\d{2}-\d{2})/)
  if (!issueMatch) {
    throw new Error('index.html does not contain a generated issue date.')
  }
...
function assertServiceWorkerFreshness() {
  ...
  const cacheDate = cacheDateMatch[1]
  if (cacheDate < issueDate) {
    throw new Error(`CACHE_NAME ${cacheName} is older than generated issue date ${issueDate}. Bump service-worker.js before refresh.`)
  }
...
function main() {
  ...
  console.log(
    `StackScout launch safety passed: ${publicFiles.length} public files scanned, CACHE_NAME=${freshness.cacheName}, issue=${freshness.issueDate}.`,
  )
}
...
try {
  ...
  console.error(`StackScout launch safety failed: ${error.message}`)
  process.exit(1)
}
